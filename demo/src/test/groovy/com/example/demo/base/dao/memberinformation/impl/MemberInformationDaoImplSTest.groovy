...
@RunWith(Enclosed)
class MemberInformationDaoImplSTest {
    @SpringBootTest
    static class TestSelect extends Specification {
        ...
        def setup() {
            ...
            databaseTester.dataSet = loader.loadDataSet(ResourceUtils.getURL("src/test/groovy/com/example/demo/base/dao/memberinformation/impl/"))
        ...
        def "selectOne動作確認: 取得データ有"() {
            ...
            when:
            def dto = memberInformationDao.selectOne(memberId)
        ...
        def "selectOneForMember: 取得データ有"() {
            ...
            when:
            def dto = memberInformationDao.selectOneForMember(memberId, memberName)
        ...
        def "selectMany動作確認: 会員ID検索：取得データ有"() {
            ...
            when:
            def dtoList = memberInformationDao.selectMany(memberId, memberName)
    ...
    static class TestInsertOne extends Specification {
        ...
        def execute() {
            memberInformationDao.insertOne(dto)
        }
    ...
    static class TestUpdateOne extends Specification {
        ...
        def execute() {
            memberInformationDao.updateOne(dto)
        }
    ...
    static class TestDeleteOne extends Specification {
        ...
        def execute() {
            memberInformationDao.deleteOne(memberId)
        }
