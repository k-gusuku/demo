...
@RunWith(Enclosed)
class MemberInformationServiceImplSTest {
    @SpringBootTest
    static class TestSelect extends Specification {
        def service = null as MemberInformationServiceImpl

        def setup() {
            service = new MemberInformationServiceImpl(Mock(MemberInformationDao))
        }
        ...
        def "selectOne: 動作確認"() {
            ...
            then:
            1 * service.memberInformationDao.selectOne({
                memberId == "100000000"
            } as String) >> new MemberInformationDto(memberId: "100000000", memberName: "会員NAME", birthday: new Date("1989/10/13"), age: 31, phoneNumber: "012223344", address: "東京都渋谷区")
            and:
            dto.each {
        ...
        def "selectOneForMember: 動作確認"() {
            ...
            then:
            1 * service.memberInformationDao.selectOneForMember(
                    { memberId == "100000000" } as String,
                    { memberName == "会員NAME" } as String
            ) >> new MemberInformationDto(memberId: "100000000", memberName: "会員NAME", birthday: new Date("1989/10/13"), age: 31, phoneNumber: "012223344", address: "東京都渋谷区")
            and:
            dto.each {
                assert it.memberId == "100000000"
                assert it.memberName == "会員NAME"
        ...
        def "selectMany: 動作確認"() {
            ...
            then:
            1 * service.memberInformationDao.selectMany(
                    { memberId == "100000000" } as String,
                    { memberName == "会員NAME" } as String
            ) >> [new MemberInformationDto(memberId: "100000001", memberName: "会員NAME1", birthday: new Date("1989/10/13"), age: 31, phoneNumber: "012223344", address: "東京都渋谷区"), new MemberInformationDto(memberId: "100000002", memberName: "会員NAME2", birthday: new Date("1990/12/25"), age: 25, phoneNumber: "012355555", address: "沖縄県那覇市")]
            and:
            dtoList[0].each {
    ...
    static class testInsertOne extends Specification {
        ...
        def dto = new MemberInformationDto()
        ...
        def "insertOne: 動作確認：登録成功"() {
            ...
            and:
            1 * service.memberInformationDao.insertOne({
                it.memberId == "100000000" &&
    ...
    static class testUpdate extends Specification {
        ...
        def "updateOne: 動作確認：更新成功"() {
            ...
            then:
            1 * service.memberInformationDao.updateOne({
                it.memberId == "100000000" &&
                        it.memberName == "会員NAME" &&
                        it.birthday == new Date("1989/10/13") &&
                        it.age == 31 &&
    ...
    static class testDeleteOne extends Specification {
        ...
        def "deleteOne: 動作確認：削除成功"() {
            ...
            then:
            1 * service.memberInformationDao.deleteOne({
                memberId == "100000000"
            } as String) >> 1
            and:
            // resultがtrueである確認
