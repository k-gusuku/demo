...
@RunWith(Enclosed)
class ProductInformationServiceImplSTest {
    @SpringBootTest
    static class TestSelect extends Specification {
        def service = null as ProductInformationServiceImpl

        def setup() {
            service = new ProductInformationServiceImpl(Mock(ProductInformationDao))
        }
        ...
        def "selectOne動作確認"() {
            ...
            then:
            1 * service.productInformationDao.selectOne({
                productId == "tetris"
            } as String) >> new ProductInformationDto(productId: "tetris", productName: "テトリス", productPrice: 2900L, productType: "ゲームソフト", productImageId: "image_tetris")
            and:
            dto.each {
        ...
        def "selectMany動作確認"() {
            ...
            then:
            1 * service.productInformationDao.selectMany(
                    { productId == "tetris" } as String,
                    { productName == "スイッチ" } as String
            ) >> [new ProductInformationDto(productId: "tetris", productName: "テトリス", productPrice: 2900L, productType: "ゲームソフト", productImageId: "image_tetris"), new ProductInformationDto(productId: "switch", productName: "スイッチ", productPrice: 29000L, productType: "その他", productImageId: "image_switch")]
            and:
            dtoList.size() == 2
            dtoList[0].each {
                assert it.productId == "tetris"
    ...
    static class testInsertOne extends Specification {
        ...
        def dto = new ProductInformationDto()
        ...
        def "insertOne動作確認: 登録成功"() {
            ...
            then:
            1 * service.productInformationDao.insertOne({
                it.productId == "tetris" &&
                        it.productName == "テトリス" &&
                        it.productPrice == 2900L &&
                        it.productType == "ゲームソフト" &&
                        it.productImageId == "image_tetris"
            } as ProductInformationDto) >> 1
            and:
            // resultがtrueである確認
            assert result
        }
    ...
    static class testUpdateOne extends Specification {
        ...
        def "insertOne動作確認: 登録成功"() {
            ...
            then:
            1 * service.productInformationDao.updateOne({
                it.productId == "tetris" &&
                        it.productName == "テトリス" &&
                        it.productPrice == 2900L &&
                        it.productType == "ゲームソフト" &&
    ...
    static class testDeleteOne extends Specification {
        ...
        def "deleteOne動作確認: 削除成功"() {
            ...
            then:
            1 * service.productInformationDao.deleteOne({
                productId == "tetris"
            } as String) >> 1
            and:
            // resultがtrueである確認
