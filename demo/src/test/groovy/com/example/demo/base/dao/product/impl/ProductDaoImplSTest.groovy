...
class ProductDaoImplSTest {
    ...
    static class TestSelect extends Specification {
        ...
        def setup() {
            ...
            databaseTester.dataSet = loader.loadDataSet(ResourceUtils.getURL("src/test/groovy/com/example/demo/base/dao/product/impl/"))
        ...
        def "selectOne動作確認: 取得データ有"() {
            ...
            when:
            def dto = productDao.selectOne(productId)
        ...
        def "selectMany動作確認: 商品名検索：完全一致：取得データ有"() {
            ...
            when:
            def dtoList = productDao.selectMany(productId, productName)
